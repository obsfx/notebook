## 📝 useful pages about programming, math and computer graphics.

#### Python

[Problem Solving with Algorithms and Data Structures using Python](https://runestone.academy/runestone/books/published/pythonds/index.html)

#### Math

[Chicago undergraduate mathematics bibliography](https://www.ocf.berkeley.edu/~abhishek/chicmath.htm)

[Mathematics for Computer Graphics](https://www.cc.gatech.edu/~turk/math_gr.html)

[Unit Vector Math for 3D Graphics](http://www.jmargolin.com/uvmath/uvmenu.htm)

#### DOS Game Dev

[Alex Russell's Dos Game Programming in C for Beginners A Tutorial](http://www3.telus.net/alexander_russell/course/introduction.htm)

#### C64 Game Dev

[C64 Game Development Resources](https://codebase64.org/doku.php?id=base:game_programming)

#### Raycasting

[Lode's Computer Graphics Tutorial Raycasting](https://lodev.org/cgtutor/raycasting.html)

[Ray-Casting Tutorial For Game Development And Other Purposes](https://permadi.com/1996/05/ray-casting-tutorial-table-of-contents/)

#### Emulator Development

[Emulator 101](http://www.emulator101.com/)

[How to write an emulator (CHIP-8 interpreter)](http://www.multigesture.net/articles/how-to-write-an-emulator-chip-8-interpreter/)

#### Socket Programming

[Sockets Programming Tutorial with C](http://www.cs.rpi.edu/~moorthy/Courses/os98/Pgms/socket.html)

[Winsock tutorial – Socket programming in C on windows](https://www.binarytides.com/winsock-socket-programming-tutorial/)

#### WebAssembly

[Introduction to WebAssembly](https://rsms.me/wasm-intro)

#### Creative Coding

[Intro to Creative Coding](https://github.com/mattdesl/workshop-p5-intro)

#### Rust

[The Rust Programming Language](https://doc.rust-lang.org/book/)

[Writing a Command Line Tool in Rust](https://mattgathu.github.io/writing-cli-app-rust/)

[CIS 198: Rust Programming University of Pennsylvania](http://cis198-2016s.github.io/schedule/)

#### General

[Quake on an oscilloscope: A technical report](http://www.lofibucket.com/articles/oscilloscope_quake.html)